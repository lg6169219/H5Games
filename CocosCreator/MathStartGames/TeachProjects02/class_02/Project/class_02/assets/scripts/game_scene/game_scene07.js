...
cc.Class({
    ...
    onLoad() {
        ...
        this.text_content.string = "下面的数字只减少一根小棍，不移动其他小棍的情况下会变成哪个新数字呢？";
    ...
    start() {
        ...
        this.sub_one_info = new_node.getComponent("common_num").get_sub_one_info();
        this.can_create_times = 0;
        if (this.sub_one_info && this.sub_one_info !== null) {
            var arr = Object.keys(this.sub_one_info);
            this.can_create_times = arr.length;
        }
        this.can_create_times--;
        while (this.can_create_times > 0) this.btn_next_click();
    },
    // 提交答案
    submitAnswer() {
        if (this.is_over) {
    ...
    change_num: function(curr_node, to_val, deltiems) {
        ...
        new_node.getComponent("common_num").opt_type = -1;
        this.node_content2.getChildByName("node_layout").insertChild(new_node, curr_node.getSiblingIndex());
    ...
    get_result: function() {
        ...
        var arr_pre = Object.keys(this.sub_one_info);
