...
cc.Class({
    ...
    tower_ai: function() {
        ...
        var enemy = ugame.search_enemy(center_pos, search_r, this.tower_type);

        var update_time = 0.1; // AI更新时间

        if (enemy) {
            var w_pos = enemy.convertToWorldSpaceAR(cc.v2(0, 0));
            this.shoot_at(w_pos);
    ...
    // 画出塔的攻击范围
    _create_tower_scope: function() {
        var node_scope = this.node.getChildByName("node_scope");
        var ctx = node_scope.getComponent(cc.Graphics);
        if (!ctx) {
            ctx = node_scope.addComponent(cc.Graphics);
        }
        ctx.clear();

        var search_r = fashi_tower_params[this.tower_level - 1].search_r;

        ctx.fillColor = new cc.Color().fromHEX('#90EE90');
        ctx.fillColor.a = 128;
        ctx.circle(0, 0, search_r);
        ctx.fill();
    },
    // update (dt) {},
});